...
One host owns a child PTY and writes the durable `.ptytrace`; joined
clients see the shared viewport and can type into the same session.
Joined clients use local `Ctrl-]` controls for detach, help, redraw, and
literal prefix input.
