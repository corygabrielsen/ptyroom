...
mod tests {
    ...
    #[test]
    fn setenv_applies_to_local_spawn_and_trace_header() {
        let script = parse(
            r#"
Version 1
SetSpawn "sh" "-c" "printf '%s' \"$PTYTRACE_SCRIPT_ENV\""
SetEnv "PTYTRACE_SCRIPT_ENV" "from-script"
...
"#,
        ...
        let trace = script.run().unwrap();

        assert_eq!(
            trace.header.env.get("PTYTRACE_SCRIPT_ENV"),
            Some(&"from-script".to_owned())
        );
        assert!(
            trace
                .events
                .iter()
                .any(|event| event.data.contains("from-script"))
        );
    }
}
