...
fn validate_env_key(key: &str, lineno: u32) -> anyhow::Result<()> {
    if key.is_empty() || key.contains('=') {
        bail!("script:{lineno}: SetEnv key must be non-empty and must not contain '='");
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn setenv_rejects_invalid_key() {
        let err = parse("Version 1\nSetSpawn \"sh\"\nSetEnv \"BAD=KEY\" \"value\"\n").unwrap_err();

        assert!(err.to_string().contains("SetEnv key"), "{err}");
    }
