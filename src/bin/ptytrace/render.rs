...
pub struct Args {
    ...
    /// matching contract via `ptytrace verify --contract`. Requires
    /// `--receipt` to be set.
...
pub fn run(args: &Args) -> anyhow::Result<()> {
    let (trace_sha256, trace_size_bytes) = super::attestation_io::trace_sha256(&args.trace)?;
    let mut r = ptytrace::render(&args.trace)?
